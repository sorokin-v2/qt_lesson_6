...
void MainWindow::StartRace(void){
    ...
    if(ui->rb_qtConcur->isChecked()){
        ...
        auto func1 = [&]{ return concurRace1->DoWork(&number, false,  ui->sb_initNum->value());};
        auto func2 = [&]{ return concurRace2->DoWork(&number, false,  ui->sb_initNum->value());};
        QtConcurrent::run(func1).then(func2);
    }
    else{
